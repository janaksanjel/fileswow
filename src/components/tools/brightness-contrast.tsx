...
export default function brightness_contrast_Tool({ onProcessing, onError }: ToolUIProps) {
...
const b = amount; const c = amount;
const factor = (259 * (c + 255)) / (255 * (259 - c));
for (let i = 0; i < d.length; i += 4) {
  d[i] = Math.min(255, Math.max(0, factor * (d[i] + b - 128) + 128));
  d[i+1] = Math.min(255, Math.max(0, factor * (d[i+1] + b - 128) + 128));
  d[i+2] = Math.min(255, Math.max(0, factor * (d[i+2] + b - 128) + 128));
}
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image to adjust brightness & contrast" />
      ) : (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-text-secondary mb-2">Brightness: {amount}</label>
            <input type="range" min={-100} max={100} value={amount} onChange={(e) => setAmount(parseInt(e.target.value))} className="w-full accent-accent" />
          </div>
