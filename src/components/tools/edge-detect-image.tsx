...
export default function edge_detect_image_Tool({ onProcessing, onError }: ToolUIProps) {
...
for (let y = 1; y < imageData.height - 1; y++) {
  for (let x = 1; x < w - 1; x++) {
    const idx = (y * w + x) * 4;
    const tl = copy[((y-1)*w+(x-1))*4], t = copy[((y-1)*w+x)*4], tr = copy[((y-1)*w+(x+1))*4];
    const l = copy[(y*w+(x-1))*4], r = copy[(y*w+(x+1))*4];
    const bl = copy[((y+1)*w+(x-1))*4], b = copy[((y+1)*w+x)*4], br = copy[((y+1)*w+(x+1))*4];
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image to detect edges" />
      ) : (
