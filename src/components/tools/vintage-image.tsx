...
export default function vintage_image_Tool({ onProcessing, onError }: ToolUIProps) {
...
for (let i = 0; i < d.length; i += 4) {
  ...
  d[i] = d[i] * 0.7 + avg * 0.3;
  d[i+1] = d[i+1] * 0.7 + avg * 0.3;
  d[i+2] = d[i+2] * 0.7 + avg * 0.3;
}
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image for vintage filter" />
      ) : (
