...
export default function saturation_adjust_Tool({ onProcessing, onError }: ToolUIProps) {
...
for (let i = 0; i < d.length; i += 4) {
  ...
  d[i] = Math.min(255, Math.max(0, gray + s*(d[i]-gray)));
  d[i+1] = Math.min(255, Math.max(0, gray + s*(d[i+1]-gray)));
  d[i+2] = Math.min(255, Math.max(0, gray + s*(d[i+2]-gray)));
}
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image to adjust saturation" />
      ) : (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-text-secondary mb-2">Saturation: {amount}</label>
