...
export default function jpg_to_webp_Tool({ onProcessing, onError }: ToolUIProps) {
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept=".jpg,.jpeg" onFilesSelected={handleFile} label="Drop an image to convert jpg to webp" description="Supports JPG,JPEG formats" />
      ) : (
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-3 rounded-lg bg-bg-elevated border border-border-base">
            <span className="w-8 h-8 rounded flex items-center justify-center text-xs font-bold bg-accent-start/10 text-accent-end">.WEBP</span>
          ...
          {result && (
            <div className="p-4 rounded-xl bg-success/[0.04] border border-success/10">
              ...
              <DownloadButton blob={result} filename={file.name.replace(/\.[^.]+$/, ".webp")} />
            </div>
          )}
