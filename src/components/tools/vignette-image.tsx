...
export default function vignette_image_Tool({ onProcessing, onError }: ToolUIProps) {
...
for (let y = 0; y < h; y++) {
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image to add vignette" />
      ) : (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-text-secondary mb-2">Vignette Strength: {amount}</label>
