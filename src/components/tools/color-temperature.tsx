...
export default function color_temperature_Tool({ onProcessing, onError }: ToolUIProps) {
...
for (let i = 0; i < d.length; i += 4) {
  d[i] = Math.min(255, Math.max(0, d[i] + t * 0.8));     // R
  d[i+2] = Math.min(255, Math.max(0, d[i+2] - t * 0.8)); // B
}
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image to adjust color temperature" />
      ) : (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-text-secondary mb-2">Temperature: {amount}</label>
            <input type="range" min={-50} max={50} value={amount} onChange={(e) => setAmount(parseInt(e.target.value))} className="w-full accent-accent" />
          </div>
