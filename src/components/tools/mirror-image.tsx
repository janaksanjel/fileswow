...
export default function mirror_image_Tool({ onProcessing, onError }: ToolUIProps) {
  ...
  const modes = [{"v":"mirror-h","l":"↔ Mirror Left-Right"},{"v":"mirror-v","l":"↕ Mirror Top-Bottom"}];

  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Mirror Image" />
      ) : (
        <div className="space-y-4">
          ...
          {!result && (
            <button onClick={process} disabled={processing} className="btn-primary w-full py-3">
              {processing ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin-slow" />Processing...</span> : "Mirror Image"}
            </button>
          )}
        </div>
      )}
