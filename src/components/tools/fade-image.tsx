...
export default function fade_image_Tool({ onProcessing, onError }: ToolUIProps) {
...
for (let i = 0; i < d.length; i += 4) {
  d[i] = Math.round(d[i] + (255 - d[i]) * fade);
  d[i+1] = Math.round(d[i+1] + (255 - d[i+1]) * fade);
  d[i+2] = Math.round(d[i+2] + (255 - d[i+2]) * fade);
}
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image to fade" />
      ) : (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-text-secondary mb-2">Fade Amount: {amount}</label>
            <input type="range" min={0} max={100} value={amount} onChange={(e) => setAmount(parseInt(e.target.value))} className="w-full accent-accent" />
          </div>
