...
export default function sketch_image_Tool({ onProcessing, onError }: ToolUIProps) {
...
// Dodge blend (simplified)
for (let i = 0; i < d.length; i += 4) {
  const v = Math.min(255, 255 * 255 / (256 - d[i] + 1));
  d[i] = d[i+1] = d[i+2] = v;
}
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image for pencil sketch effect" />
      ) : (
