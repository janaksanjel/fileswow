...
export default function PdfToWordTool({ onProcessing, onError }: ToolUIProps) {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      paragraphs.push(new Paragraph({
        children: [new TextRun({ text: `Converted from: ${file.name}`, bold: true, size: 24 })],
        heading: HeadingLevel.HEADING_1,
      }));
      paragraphs.push(new Paragraph({ children: [new TextRun({ text: `Pages: ${doc.getPageCount()}`, size: 20 })] }));

      for (let i = 0; i < doc.getPageCount(); i++) {
        paragraphs.push(new Paragraph({
          children: [new TextRun({ text: `Page ${i + 1}`, bold: true, size: 22 })],
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept=".pdf" onFilesSelected={handleFile} label="Drop a PDF" description="Convert PDF to Word DOCX" />
      ) : (
