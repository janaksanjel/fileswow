...
export default function cartoon_image_Tool({ onProcessing, onError }: ToolUIProps) {
...
for (let y = 1; y < h - 1; y++) {
  for (let x = 1; x < w - 1; x++) {
    ...
    if (Math.sqrt(gx*gx+gy*gy) > 80) { d[idx] = d[idx+1] = d[idx+2] = 0; }
  }
}
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image for cartoon effect" />
      ) : (
