...
export default function opacity_adjust_Tool({ onProcessing, onError }: ToolUIProps) {
...
for (let i = 0; i < d.length; i += 4) {
  d[i+3] = Math.round(Math.min(255, Math.max(0, alpha * 255)));
}
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image to change opacity" />
      ) : (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-text-secondary mb-2">Opacity: {amount}</label>
