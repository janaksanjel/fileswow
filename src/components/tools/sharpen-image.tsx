...
export default function sharpen_image_Tool({ onProcessing, onError }: ToolUIProps) {
...
for (let y = 1; y < imageData.height - 1; y++) {
  for (let x = 1; x < w - 1; x++) {
    ...
    for (let c = 0; c < 3; c++) {
      ...
      d[idx+c] = Math.min(255, Math.max(0, copy[idx+c] + (center - neighbors) * strength * 0.5));
    }
  }
}
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image to sharpen" />
      ) : (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-text-secondary mb-2">Sharpen Amount: {amount}</label>
