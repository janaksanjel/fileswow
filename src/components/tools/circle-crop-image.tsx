...
export default function circle_crop_image_Tool({ onProcessing, onError }: ToolUIProps) {
  ...
  return (
    <div className="space-y-6">
      {!file ? <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image for circle crop" /> : (
        <div className="space-y-4">
          ...
          {result && <div className="p-4 rounded-xl bg-success/[0.04] border border-success/10"><p className="text-sm text-success mb-1">✓ Circle cropped!</p><DownloadButton blob={result} filename={"circle-" + file.name} /></div>}
          {!result && <button onClick={process} disabled={processing} className="btn-primary w-full py-3">{processing ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin-slow" />Processing...</span> : "Circle Crop"}</button>}
        </div>
      )}
    </div>
  );
