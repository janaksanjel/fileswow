...
export default function pixelate_image_Tool({ onProcessing, onError }: ToolUIProps) {
...
const blockSize = Math.max(2, Math.round(amount / 2));
for (let y = 0; y < h; y += blockSize) {
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image to pixelate" />
      ) : (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-text-secondary mb-2">Pixel Size: {amount}</label>
            <input type="range" min={2} max={50} value={amount} onChange={(e) => setAmount(parseInt(e.target.value))} className="w-full accent-accent" />
          </div>
