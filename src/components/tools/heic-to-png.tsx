...
export default function heic_to_png_Tool({ onProcessing, onError }: ToolUIProps) {
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept=".heic,.heif" onFilesSelected={handleFile} label="Drop an image to convert heic to png" description="Supports HEIC,HEIF formats" />
      ) : (
        <div className="space-y-4">
          ...
          {result && (
            <div className="p-4 rounded-xl bg-success/[0.04] border border-success/10">
              ...
              <DownloadButton blob={result} filename={file.name.replace(/\.[^.]+$/, ".png")} />
            </div>
          )}
