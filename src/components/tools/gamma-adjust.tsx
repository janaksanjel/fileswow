...
export default function gamma_adjust_Tool({ onProcessing, onError }: ToolUIProps) {
...
const gamma = Math.max(0.1, amount / 50 + 1);
...
for (let i = 0; i < d.length; i += 4) {
  ...
  return (
    <div className="space-y-6">
      {!file ? (
        <DropZone accept="image/*" onFilesSelected={handleFile} label="Drop an image for gamma correction" />
      ) : (
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-text-secondary mb-2">Gamma: {amount}</label>
